...
{this.p_updateSlideshow();this.p_updateDownloadVisibility();this.p_addEvent(document,'onkeydown',this.p_keyDown.bind(this),true);this.p_addEvent(document,'onkeyup',this.p_keyUp.bind(this),true);this.div().getElementsByClassName("noselect").each(function(element)
...
{this.p_updateCanvasControls();this.p_setupThumbnails();this.mSlideshow.showPhotoNumber(this.p_startIndex());var captionDiv=this.div().getElementsByClassName("Caption")[0];captionDiv.innerHTML="&#160;";captionDiv.id="caption";}}
...
{var thumbnailView=this.getElementById("thumbnail_view");var back=this.getElementById("thumbnails_back");var forward=this.getElementById("thumbnails_forward");var viewSpan=this.getElementById("view");var thumbsOnOnly=viewSpan.getElementsByClassName("thumbs_on_only");var thumbsOffOnly=viewSpan.getElementsByClassName("thumbs_off_only");var thumbnailViewFullHeight=59;if(show)
...
{var selected=thumbnailsDiv.getElementsByClassName("thumbnail")[index];if(selected)
...
{var enableForward=false;var thumbnails=thumbnailsDiv.getElementsByClassName("thumbnail");if(thumbnails&&thumbnails.length>0)
...
{this.div().getElementsByClassName("play_slideshow").invoke(this.p_showSlideshow()?'show':'hide');}
...
{var captionDiv=this.div().getElementsByClassName("Caption")[0];captionDiv.style.height=this.preferenceForKey("captionHeight")+"px";}
...
{var thumbnailsDiv=this.getElementById("thumbnails");var selectedThumbContainer=thumbnailsDiv.getElementsByClassName("selected")[0];var img=selectedThumbContainer.getElementsByTagName("img")[0];this.p_setThumbnail(img,entry.thumbnail());}}
...
{var canvasControlURLs=this.preferenceForKey("canvas controls");$(this.div()).getElementsByClassName('canvas').each(function(img)
...
var captionDiv=this.div().getElementsByClassName("Caption")[0];if(captionDiv)
{captionDiv.update(currentEntry.title());}}
var thumbnailsDiv=this.getElementById("thumbnails");var oldSelected=thumbnailsDiv.getElementsByClassName("selected")[0];if(oldSelected)
{oldSelected.removeClassName("selected");}
var newSelected=thumbnailsDiv.getElementsByClassName("thumbnail")[index];if(newSelected)
...
{var thumbnailsDiv=this.getElementById("thumbnails");var thumbnails=thumbnailsDiv.getElementsByClassName("thumbnail");var lastThumbnail=thumbnails[thumbnails.length-1];var lastThumbnailRight=lastThumbnail.offsetLeft+lastThumbnail.offsetWidth;return lastThumbnailRight;}
